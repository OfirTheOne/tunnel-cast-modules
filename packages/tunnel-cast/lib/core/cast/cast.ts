...
export function cast<T>(model: new (...args: any[]) => T, target: any, options: CastOptions = {}): CastResult<T> {
    ...
    const decoratedModelFieldsSet: Map<string, number> = Reflect.getMetadata(
        CAST_DECORATED_FIELDS_LIST,
        modelPrototype,
    );
    const orderedDecoratedFields = Array.from(decoratedModelFieldsSet?.entries() || [])
        .sort(([, v1], [, v2]) => v1 - v2)
        .map(([v]) => v);

    const castProceduresListEntries: Array<[string, FieldProcedure[]]> = orderedDecoratedFields.map((field) => {
...
export function castEngine(
    castProceduresListEntries: Array<[string, FieldProcedure[]]>,
    target: any,
    ...
    const messagesLists = castProceduresListEntries.map(([field, proceduresList]) => {
        ...
        const proceduresExecutionResult = executeCastProcedures(
            field,
            proceduresListGroups,
            target,
            projectedContext,
            options,
        );
