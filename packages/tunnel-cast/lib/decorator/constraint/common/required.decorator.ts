...
export const required: FieldConstraintFn<{}> = ({ fieldValue }) =>
    !globalSetting.defaultEmptyIdentifier({ fieldValue } as any);
export const requiredMessageBuilder: MessageBuilderFn = ({ fieldName, options }) =>
    options.iterate ? `Each value in the field ${fieldName} is required.` : `The field ${fieldName} is required.`;
...
export function Required(options?: FieldConstraintProcedureOptions) {
    const adaptee = new FieldConstraintProcedure(REQUIRED, options || {}, {}, required, requiredMessageBuilder);
