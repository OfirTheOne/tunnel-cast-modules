...
const arrayStartsWithItems = (array: Array<any>, prefixItems: Array<any>) => (
    (array.length >= prefixItems.length) ?
        prefixItems.every((item, i) => array[i] === item ) :
        false
);
...
export const startsWithMessageBuilder = ({ fieldName }) => `The field ${fieldName} dose not start with the provided value.`;
